...
print_stackframe(void) {
    ...
    int i, j;
    for(i = 0; ebpV != 0 && i < STACKFRAME_DEPTH; i++){ // 若不加入ebpV != 0 的限制，则会多出许多ebpV = 0时的输出
	cprintf("ebp:%08x eip:%08x ", ebpV, eipV);	// 打印ebp，eip
	cprintf("args: ");
	uint32_t *args = (uint32_t *)ebpV + 2;		// ebp+2 得到最后入栈的参数地址
	for(j = 0; j < 4; j ++) {
	    cprintf("0x%08x ", args[j]);    		// 依次打印参数    
	}
	cprintf("\n");
	print_debuginfo(eipV - 1);			// 调用提示函数打印当前函数信息
	eipV = ((uint32_t *)ebpV + 1)[0];		// 得到下一次打印所需的ebp eip的值
        ebpV = ((uint32_t *)ebpV)[0];
    }
}
